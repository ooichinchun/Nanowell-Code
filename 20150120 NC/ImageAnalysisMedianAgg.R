...
#cur_dir = choose.dir()
#setwd(cur_dir)
setwd("C:/Lab/Patient Samples Nanowell/New Median Testing with Rolling Ball/20150120 NC")

CD45 = read.csv("Before 5X_F_CY3 RB.csv",header=TRUE)
PE = read.csv("After PCR_5X F_CY3 RB.csv",header=TRUE)
FITC = read.csv("After PCR_5X F_GFP RB.csv",header=TRUE)

PE = subset(PE,!(is.na(CD45$O_75th)))
FITC = subset(FITC,!(is.na(CD45$O_75th)))
CD45 = subset(CD45,!(is.na(CD45$O_75th)))

FITC = subset(FITC,!(is.na(PE$O_75th)))
CD45 = subset(CD45,!(is.na(PE$O_75th)))
PE = subset(PE,!(is.na(PE$O_75th)))

PE = subset(PE,!(is.na(FITC$O_75th)))
CD45 = subset(CD45,!(is.na(FITC$O_75th)))
FITC = subset(FITC,!(is.na(FITC$O_75th)))

Sig_Value = abs(qt(0.05/nrow(CD45),nrow(CD45)))

CD45.corr = (CD45$O_75th-CD45$Sq_Mean)/CD45$Sq_Mean
#CD45.corr = (CD45$O_75th-CD45$Sq_Med)/CD45$Sq_Med

#Med.corr = CD45_Agg$CD45_Mean
#SD.corr = CD45_Agg$CD45_SD

Med.corr = median(CD45.corr)
SD.corr = sd(CD45.corr)
#SD.corr = Sn(CD45.corr)
...
#PE_B.corr = (PE_B$O_75th-PE_B$Sq_Med)/PE_B$Sq_Med

#PE_B.Med.corr = PE_Agg$PE_Mean
PE_B.Med.corr = median(PE_B.corr)

#PE_B.SD.corr = (PE_Agg$PE_SD+PE_Agg$PE_SN)/2
PE_B.SD.corr = sd(PE_B.corr)
#PE_B.SD.corr = Sn(PE_B.corr)
...
#FITC_B.corr = (FITC_B$O_75th-FITC_B$Sq_Med)/FITC_B$Sq_Med

#FITC_B.Med.corr = FITC_Agg$FITC_Mean
FITC_B.Med.corr = median(FITC_B.corr)

#FITC_B.SD.corr = (FITC_Agg$FITC_SD+FITC_Agg$FITC_SN)/2
FITC_B.SD.corr = sd(FITC_B.corr)
#FITC_B.SD.corr = Sn(FITC_B.corr)
...
plot(x=FITC_B.corr,y=PE_B.corr,ylab = "PE Intensity (A.U.)",xlab = "FITC Intensity (A.U.)",main="Post-PCR (Mean subtracted)",,col=All_B.possible,xlim=c(-1,10),ylim=c(-1,12))
